...
const PublicSite: React.FC<PublicSiteProps> = ({ blueprint }) => {
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-slate-900 animate-fade-in">
      ...
      {/* Pricing & Features Omitted for Brevity - Keeping Core Lead Capture */}
                ...
                <form onSubmit={handleSubmit} className="bg-white/5 p-8 md:p-10 rounded-[2rem] border border-white/10 backdrop-blur-xl text-left space-y-6 shadow-2xl">
                    <div className="grid md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input required type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full px-5 py-4 rounded-xl bg-slate-900/50 border border-slate-700 focus:ring-2 focus:ring-indigo-500 outline-none text-white transition-all" placeholder="John" />
                        </div>
                        <div>
                            <label className="block text-sm font-bold mb-2 ml-1 text-slate-300">Email Address</label>
                            <input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-5 py-4 rounded-xl bg-slate-900/50 border border-slate-700 focus:ring-2 focus:ring-indigo-500 outline-none text-white transition-all" placeholder="john@example.com" />
                        </div>
                    </div>
                    {/* New Phone Field */}
                    <div>
                        <label className="block text-sm font-bold mb-2 ml-1 text-slate-300">Phone Number (Optional)</label>
                        <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full px-5 py-4 rounded-xl bg-slate-900/50 border border-slate-700 focus:ring-2 focus:ring-indigo-500 outline-none text-white transition-all" placeholder="+1 (555) 000-0000" />
                    </div>
