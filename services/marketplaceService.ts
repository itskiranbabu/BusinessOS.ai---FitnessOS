...
let MOCK_TEMPLATES: Template[] = [
  ...
  // ADMIN TEMPLATE (For testing Edit/Delete)
  {
    id: '3',
    author_id: 'mock-user-123', // Matches authService mock user ID
    title: 'My Custom System',
    description: 'A template created by you. Edit or delete this to test admin features.',
    category: 'full_system',
    niche: 'Wellness',
    price: 9900,
    is_public: true,
...
export const marketplaceService = {
  ...
  getTemplates: async (category?: TemplateCategory): Promise<Template[]> => {
    if (!isSupabaseConfigured() || !supabase) return MOCK_TEMPLATES.filter(t => category ? t.category === category : true);
  ...
  getAuthoredTemplates: async (): Promise<Template[]> => {
      ...
      if (!isSupabaseConfigured() || !supabase) {
          // For mock mode, filter by the mock user ID
          return MOCK_TEMPLATES.filter(t => t.author_id === 'mock-user-123');
      }
  ...
  publishTemplate: async (
    ...
    if (!isSupabaseConfigured() || !supabase) {
        console.warn("Supabase not connected. Using Mock Publish.");
